...
public class MySQLProvider implements IConnectionProvider {
    ...
    public MySQLProvider(@NotNull ConnectionSettings config) throws ClassNotFoundException {
        ...
        this.LINK = "jdbc:mysql://"+(config.getHost().contains(":")?config.getHost() : config.getHost()+":3306") +"/"+config.getDatabase()+(config.getDatabase().contains("autoReconnect") ? "": config.getDatabase().contains("?") ? "&autoReconnect=true" : "?autoReconnect=false");
